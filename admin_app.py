...
.zf-card{border:1px solid #e4e7ec;border-radius:18px;padding:18px;background:#fff}
"""
...
def admin_refresh(uid,token,csrf):
    return base.admin_refresh_view(uid,token,csrf)


def search(uid,token,csrf,keyword):
    return base.admin_search_view(uid,token,csrf,keyword)


def users_all(uid,token,csrf):
    return base.admin_search_view(uid,token,csrf,"")


def usage(uid,token,csrf):
    return base.admin_usage_view(uid,token,csrf)
...
def recharge_orders(uid,token,csrf,status):
    return base.admin_recharge_orders(uid,token,csrf,status)


def recharge_review(uid,token,csrf,order_id,approve,note):
    msg,rows=base.admin_recharge_review(uid,token,csrf,order_id,approve,note)
    return msg,rows


def adjust(uid,token,csrf,target,amount,mode):
    return base.admin_adjust(uid,token,csrf,target,amount,mode)


def set_credits(uid,token,csrf,target,amount):
    return base.admin_set(uid,token,csrf,target,amount)


def disable(uid,token,csrf,target,disabled):
    return base.admin_disable(uid,token,csrf,target,disabled)
...
with gr.Blocks(title="ZOLFOX 管理后台",theme=gr.themes.Soft(),css=CSS) as admin_app:
    ...
    with gr.Column(elem_classes=["zf-admin"]):
        ...
        with gr.Column(visible=False) as panel:
            ...
            with gr.Tabs():
                with gr.Tab("👥 用户管理"):
                    with gr.Row():
                        keyword=gr.Textbox(label="搜索用户名")
                    ...
                    with gr.Row():
                        ...
                        amount=gr.Number(label="积分",value=10,precision=0)
                        mode=gr.Radio(["增加","扣除"],value="增加",label="操作")
                        adjust_btn=gr.Button("执行")
                    with gr.Row():
                        set_amount=gr.Number(label="设置为",precision=0)
                        set_btn=gr.Button("设置积分")
                        disable_btn=gr.Button("禁用用户")
                        enable_btn=gr.Button("启用用户")
                    user_message=gr.Markdown()

                with gr.Tab("💰 充值审核"):
                    with gr.Row():
                        recharge_filter=gr.Radio(["all","pending","approved","rejected"],value="pending",label="订单状态")
                        recharge_refresh=gr.Button("刷新充值订单")
                    recharge_table=gr.Dataframe(headers=["ID","订单号","用户ID","用户名","积分","金额","付款说明","状态","审核管理员","管理员备注","申请时间","审核时间"],interactive=False)
                    with gr.Row():
                        order_id=gr.Number(label="订单 ID",precision=0)
                        approve_btn=gr.Button("✅ 通过并增加积分",variant="primary")
                        reject_btn=gr.Button("❌ 拒绝申请")
                    admin_note=gr.Textbox(label="管理员备注",max_lines=2)
                    recharge_message=gr.Markdown()

                with gr.Tab("📋 使用记录"):
                    usage_btn=gr.Button("查看全部记录")
                    logs_table=gr.Dataframe(headers=["ID","用户","引擎","积分","状态","详情","时间"],interactive=False)

                with gr.Tab("🔐 操作审计"):
                    audit_btn=gr.Button("查看审计记录")
                    audit_table=gr.Dataframe(headers=["ID","管理员","操作","目标用户","详情","时间"],interactive=False)
    ...
    gr.on(
        ...
        outputs=[user_id,session_token,csrf_token,panel,admin_message,gr.skip(),admin_stat,users_table,logs_table,audit_table],
        fn=restore,
    )
