...
def _mask_rle(mask: np.ndarray) -> str:
    """Compact row-wise RLE: rle:w:h:row,start-end;... ."""
...
(() => {
  ...
  // PNG base64 was unreliable for large images because the hidden Gradio
  // textbox could lag behind the canvas. RLE is tiny for normal brush masks
  // and is plain text, so Gradio can reliably serialize it.
  ...
  function syncMask(state) {
    ...
    box.dispatchEvent(new InputEvent('input', {bubbles:true, inputType:'insertText', data:null}));
  ...
  function install(root) {
    ...
    const state = {root, maskCanvas, mctx, payload, overlay, ctx, img, mode:'paint', drawing:false, last:null, scale:1, undo:[], redo:[], lastMaskValue:''};
    ...
    function render() {
      ...
      for (let i=0; i<d.length; i+=4) {
        ...
        out.data[i] = 255; out.data[i+1] = 40; out.data[i+2] = 40; out.data[i+3] = a;
      }
    ...
    function point(e) {
      ...
      return {x:Math.max(0,Math.min(payload.w,(e.clientX-r.left)/state.scale)), y:Math.max(0,Math.min(payload.h,(e.clientY-r.top)/state.scale))};
    }
    ...
    function stroke(a,b) {
      ...
      if (state.mode==='paint') { mctx.globalCompositeOperation='source-over'; mctx.strokeStyle='#fff'; }
      else { mctx.globalCompositeOperation='destination-out'; mctx.strokeStyle='#000'; }
      mctx.beginPath(); mctx.moveTo(a.x,a.y); mctx.lineTo(b.x,b.y); mctx.stroke(); mctx.restore();
      render();
    }

    overlay.addEventListener('pointerdown', e => {
      e.preventDefault(); snapshot(); state.drawing=true; state.last=point(e); stroke(state.last,state.last); overlay.setPointerCapture(e.pointerId);
    });
    overlay.addEventListener('pointermove', e => {
      if (!state.drawing) return; e.preventDefault(); const q=point(e); stroke(state.last,q); state.last=q;
    });
    function end(e) {
    ...
    root.querySelector('.wm-size').addEventListener('input', e => root.querySelector('.wm-size-label').textContent=e.target.value);
    root.querySelector('[data-action="undo"]').addEventListener('click', () => {
      if (!state.undo.length) return; state.redo.push(mctx.getImageData(0,0,payload.w,payload.h)); mctx.putImageData(state.undo.pop(),0,0); render(); syncMask(state);
    });
    root.querySelector('[data-action="redo"]').addEventListener('click', () => {
      if (!state.redo.length) return; state.undo.push(mctx.getImageData(0,0,payload.w,payload.h)); mctx.putImageData(state.redo.pop(),0,0); render(); syncMask(state);
    });
    root.querySelector('[data-action="clear"]').addEventListener('click', () => {
      snapshot(); mctx.clearRect(0,0,payload.w,payload.h); render(); syncMask(state);
    });

    img.onload=()=>fit();
    img.src='data:image/png;base64,'+payload.image;
    const maskImg=new Image();
    maskImg.onload=()=>{mctx.clearRect(0,0,payload.w,payload.h);mctx.drawImage(maskImg,0,0,payload.w,payload.h);render();syncMask(state);};
  ...
  function scan(){document.querySelectorAll('.wm-editor[data-payload]').forEach(install);}
  const observer=new MutationObserver(scan);
  observer.observe(document.documentElement,{childList:true,subtree:true});
  scan(); window.addEventListener('resize',scan);
})();
</script>
"""
...
def build_editor(image: Image.Image, mask: np.ndarray) -> str:
  ...
  <div class="wm-toolbar">
    ...
    <span class="wm-mask-count">Mask: 0 px</span><span class="wm-help">红色 = Mask　画笔增加　橡皮擦删除</span>
  </div>
</div>'''
...
def decode_mask(data, size):
    ...
    try:
        if data.startswith('rle:'):
            parts=data.split(':',3)
            if len(parts)!=4: raise ValueError('invalid RLE')
            w,h=int(parts[1]),int(parts[2])
            if (w,h)!=(size[0],size[1]): raise ValueError(f'RLE size {w}x{h} != image {size[0]}x{size[1]}')
            mask=np.zeros((h,w),dtype=np.uint8)
            rows=parts[3]
            if rows:
                for row in rows.split(';'):
                    if not row: continue
                    y_s,runs_s=row.split(':',1); y=int(y_s)
                    if y<0 or y>=h: continue
                    for run in runs_s.split(','):
                        a_s,b_s=run.split('-',1); a=max(0,int(a_s)); b=min(w-1,int(b_s))
                        if b>=a: mask[y,a:b+1]=255
            return mask
        if data.startswith('data:image'):
            data=data.split(',',1)[1]
        arr=np.asarray(Image.open(io.BytesIO(base64.b64decode(data))).convert('L'))
        if arr.shape!=(size[1],size[0]): arr=np.asarray(Image.fromarray(arr).resize(size,Image.Resampling.NEAREST))
...
def auto_detect(image):
    if image is None: return None,None,'⚠️ 请先上传图片。',''
    try:
        pil=_pil(image)
        mask=np.asarray(detect_candidates(pil),dtype=np.uint8)
        if mask.shape!=(pil.height,pil.width): mask=np.asarray(Image.fromarray(mask,'L').resize(pil.size,Image.Resampling.NEAREST))
        mask=np.where(mask>30,255,0).astype(np.uint8)
        return overlay_mask(pil,mask),build_editor(pil,mask),f'✅ 自动识别完成：{int(np.count_nonzero(mask)):,} 个 Mask 像素。',_mask_rle(mask)
...
def restore(image,mask_data):
    ...
    if pixels==0: raise gr.Error('Mask 是空的：请先自动识别，或在编辑器中用画笔涂满水印区域。')
