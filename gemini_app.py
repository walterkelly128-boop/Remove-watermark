...
with gr.Blocks(title="AI 图片智能修复",theme=gr.themes.Soft(),css=CSS+CARD_CSS,head=EDITOR_JS) as demo:
   ...
   with gr.Tab("🖼️ 图片修复"):
    with gr.Row():
     with gr.Column():
      ...
      with gr.Row(): auto_btn=gr.Button("✨ 自动识别候选区域",variant="primary",elem_id="wm-auto-detect"); clear_btn=gr.Button("清除 Mask",elem_id="wm-clear-mask")
      status=gr.Markdown("上传图片后开始。",elem_id="wm-status")
     with gr.Column(): preview=gr.Image(label="自动识别预览（红色=候选区域）",type="pil",elem_id="wm-preview")
    gr.Markdown("## Mask 编辑器"); editor=gr.HTML(label="Mask 编辑器",elem_id="wm-editor"); mask_data=gr.Textbox(label="",elem_id="mask-data",visible=True,container=False)
 ...
 source.change(reset_editor,source,[editor,mask_data]); clear_btn.click(reset_editor,source,[editor,mask_data]); demo.load(refresh_status,[local_status,local_desc,gemini_status,gemini_desc,openai_status,openai_desc])
