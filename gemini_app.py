...
def _engine_status(p):
 ...
 e=RCImageEngine(p)
 return ("● 已配置" if e.api_key else "○ 未配置",f"{p.title()} · {e.model} · 每次 {PROVIDER_COST[p]} 次额度")
def refresh_status():
...
def auth_login(username,password):
 u,m=accounts.login(username,password)
 return ((u["id"],gr.update(visible=False),gr.update(visible=True),f"✅ 欢迎回来，**{u['username']}**",account_text(u),gr.update(visible=bool(u['is_admin']))) if u else (None,gr.update(visible=True),gr.update(visible=False),f"❌ {m}","",gr.update(visible=False)))
...
def admin_adjust(uid,target,amount,mode):
 ...
 try:
  ...
  accounts.add_credits(target,amount if mode=="增加" else -amount)
  return f"✅ {u['username']} 当前额度：{accounts.get_user(target)['credits']}",admin_users_view(),admin_logs()
...
def admin_set(uid,target,amount):
 ...
 try:
  ...
  if not u:return "❌ 用户不存在。",[],[]
  if u["is_admin"] or target==uid:return "❌ 不能修改管理员账户。",[],[]
  accounts.set_credits(target,max(0,int(amount))); return f"✅ 已设置 {u['username']} 为 {accounts.get_user(target)['credits']} 次。",admin_users_view(),admin_logs()
 except Exception as e:return f"❌ 操作失败：{e}",admin_users_view(),admin_logs()
def admin_disable(uid,target,disabled):
 ...
 try:
  ...
  accounts.set_disabled(target,disabled); return f"✅ {u['username']} 已{'禁用' if disabled else '启用'}。",admin_users_view()
...
with gr.Blocks(title="AI 图片智能修复",theme=gr.themes.Soft(),css=CSS+CARD_CSS,head=EDITOR_JS) as demo:
 ...
 with gr.Column(visible=True) as auth_panel:
  ...
  with gr.Tabs():
   ...
   with gr.Tab("注册"):
    reg_user=gr.Textbox(label="用户名（3-32位）"); reg_pass=gr.Textbox(label="密码（至少6位）",type="password"); reg_btn=gr.Button("注册",variant="primary")
 ...
 login_btn.click(auth_login,[login_user,login_pass],[user_id,auth_panel,app_panel,auth_message,account_info,admin_tab]); reg_btn.click(auth_register,[reg_user,reg_pass],[auth_message,login_user]); logout_btn.click(logout,outputs=[user_id,auth_panel,app_panel,auth_message,account_info,admin_tab])
 ...
 admin_search_btn.click(lambda q:admin_users_view(q),admin_search,users_table); admin_all_btn.click(lambda:admin_users_view(),outputs=users_table); admin_usage_btn.click(lambda:admin_logs(),outputs=logs_table)
